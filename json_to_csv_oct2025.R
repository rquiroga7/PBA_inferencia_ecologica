...
# Paths - UPDATE THIS WITH YOUR ACTUAL JSON FILE NAME
json_file <- "data/electores_circuitos_oct2025.json"
out_file <- "data/electores_circuitos_oct2025.csv"
...
for (i in seq_len(length(json_data))) {
  ...
  if (!is.null(circuit$valoresTotalizadosPositivos) && length(circuit$valoresTotalizadosPositivos) > 0) {
    ...
    for (j in seq_len(length(positivos))) {
      ...
      # UPDATED PARTY MAPPINGS FOR OCTOBER 2025:
      # - ALIANZA FUERZA PATRIA (formerly UxP) -> FP
      # - ALIANZA LA LIBERTAD AVANZA -> LLA (no longer coalition with JxC)
      # - Everything else -> OTROS
      if (nombreAgrupacion == "ALIANZA FUERZA PATRIA") nombreAgrupacion <- "FP"
      if (nombreAgrupacion == "ALIANZA LA LIBERTAD AVANZA") nombreAgrupacion <- "LLA"
      if (!nombreAgrupacion %in% c("EN BLANCO", "FP", "LLA")) nombreAgrupacion <- "OTROS"
